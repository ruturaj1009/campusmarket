...
module.exports.likeProducts = async (req, res) => {
    try {
      ...
      const alreadyExist = await Users.exists({ _id: userId, likedProducts: productId });
  
      if (alreadyExist) {
        // If already liked, remove the product from the likedProducts array
        await Users.updateOne({ _id: userId }, { $pull: { likedProducts: productId } });
        res.send({ message: 'Product unliked successfully.' });
      } else {
        // If not liked, add the product to the likedProducts array
